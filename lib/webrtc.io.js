...
var DataChannel = function()
{
  ...
  dc.addEventListener = function(type, listener)
  {
    dc._events[type] = dc._events[type] || [];
    dc._events[type].push(callback);
  };

  dc.dispatchEvent = function(event)
  {
    var events = dc._events[event];
    if(!events)
      return;
  ...
  dc.removeEventListener = function(type, listener)
  {
    var events = dc._events[event];
    if(!events)
      return;
