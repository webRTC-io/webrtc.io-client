...
(function() {
  ...
  rtc.createPeerConnection = function(id) {
    ...
    if (rtc.dataChannelSupport) {
      pc.ondatachannel = function(evt) {
        if (debug) console.log('data channel connecting ' + id);
  ...
  rtc.attachStream = function(stream, domId) {
    ...
    if (navigator.mozGetUserMedia) {
      if (debug) console.log("Attaching media stream");
  ...
  rtc.createDataChannel = function(pcOrId, label) {
    ...
    var channel;
    try {
      if (debug) console.log('createDataChannel ' + id);
      channel = pc.createDataChannel(label, options);
    } catch (error) {
      if (debug) console.log('seems that DataChannel is NOT actually supported!');
      throw error;
    }
  ...
  rtc.addDataChannel = function(id, channel) {
    ...
    channel.onopen = function() {
      if (debug) console.log('data stream open ' + id);
    ...
    channel.onclose = function(event) {
      ...
      delete rtc.peerConnections[id];
      delete rtc.connections[id];
      if (debug) console.log('data stream close ' + id);
      rtc.fire('data stream close', channel);
    };

    channel.onmessage = function(message) {
      if (debug) console.log('data stream message ' + id);
      rtc.fire('data stream data', channel, message.data);
    };

    channel.onerror = function(err) {
      if (debug) console.log('data stream error ' + id + ': ' + err);
